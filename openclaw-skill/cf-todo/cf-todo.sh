...
case "$action" in
  ...
  todos:create)
    ...
    curl "${curl_opts[@]}" -X POST "$BASE_URL/api/v1/todos" \
      -d "{\"date\":\"$date\",\"text\":$(printf '%s' "$text" | jq -Rs .),\"priority\":\"$priority\"}"
    ;;
  ...
  cats:create)
    ...
    curl "${curl_opts[@]}" -X POST "$BASE_URL/api/v1/categories" \
      -d "{\"name\":$(printf '%s' "$name" | jq -Rs .),\"color\":\"$color\"}"
    ;;
  ...
  *)
    ...
    echo "Available: todos:date, todos:range, todos:get, todos:create, todos:update, todos:toggle, todos:delete, cats:list, cats:get, cats:create, cats:update, cats:delete" >&2
    exit 1
    ;;
esac
