...
## Changelog: <br>
- **1.5.1** (2026-06-29): 同步 API_Wiki.md 文档修订。`scope=this` 描述修订：原描述"detaches from series if `repeat_type` changes to `none`"误导，实际行为是 scope=this 总是脱离系列（对齐 Google Calendar / RFC 5545 "仅此项"语义），即使只改 `text` 等非重复字段也会脱离。`scope=all` 补充未持久化实例说明：服务端 UPDATE 只命中当前 DB 中的实例，未被 GET 触发自动展开的未来日期实例不在 DB 中不会被命中，但模板更新后未来 GET 时按新模板展开，最终行为正确。 <br>
