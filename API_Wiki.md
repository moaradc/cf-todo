...
- **PATCH /api/v1/todos/:id/toggle**
  ...
  - **响应**:
    ...
    {
      "success": true,
      "data": { "id": "uuid", "done": true, "date": "2026-06-30", "time_records": [...] }
    }
    ```
    - 仅当 `done: true` 且有 `date` / `time_records` 时才附带这些字段。
...
| 404 | 资源不存在 |
| 405 | 请求方法不允许 |
| 409 | 冲突（存在进行中的导入/导出会话；D1 UNIQUE 约束冲突） |
| 429 | 登录失败次数过多（IP 锁定 15 分钟） |
| 500 | 服务器内部错误 |
...
- **GET /api/todos?date=YYYY-MM-DD**
  ...
  - **响应**: Todo 对象数组（V0 格式，见 [4.2 节](#42-todo-对象v0-响应格式)）。
...
- **POST /api/todo-action**
  ...
  - **支持的 Action**:
    | Action | 说明 |
    |--------|------|
    | `CREATE` | 创建任务。`task.id` 必填。若 `type='recurring'`，同时创建模板。`task.parent_id` 可省略（CREATE 时后端始终用 `task.id` 作为 `parent_id`）。响应仅 `{"success":true}`，需后续 GET 获取完整对象 |
    | `UPDATE` | 更新任务（PATCH 语义，详见 [§3.2 V0 UPDATE 字段语义](#v0-update-字段语义重要)）。支持 `scope` 参数控制重复任务更新范围。**字段回退**：未传的字段（`text`/`time`/`priority`/`desc`/`url`/`copy_text`/`rrule`/`anchor_date`/`exdates` 等）从 DB 当前值回退，不会被清空。`scope=all` 缺 `task.parent_id` 时后端从 DB 派生 |
    ...
    | `TIMER_COMPLETE` | 计时"完成"：标记 done=1 + 追加 session 到 `time_records`。实例级：碎时记不截断 / 普通 todo FIFO 5；模板级 FIFO 10（仅真实耗时且非碎时记）。需 `task.id`、`parent_id`（顶层或 `task.parent_id`）、`record`，服务端校验时长合理性（1s ~ 7d）。碎时记完成时同时冻结 `date` 到 `body.date`（已校正不超过今天） |
    ...
    | `BATCH_TOGGLE_DONE` | 批量切换状态（需 `ids` + `done_status`）。`done_status: true` 可附带 `timer_records` 数组（每项含 `id` / `parent_id` / `record`），每条 record 写入规则同 `TOGGLE_DONE`；`done_status: false` 清空所有选中项实例级 `time_records`；碎时记 `date` 从 `fragment_anchor` 恢复 |
    | `BATCH_DELETE` | 批量软删除（需 `ids`），自动为 `type='recurring'` 任务添加 exdate（碎时记跳过） |
...
- **POST /api/category-action**
  ...
  - **Action 详情**:
    ...
    | `BATCH_DELETE` | 批量删除分类，关联的 Todo 和模板 `category_id` 自动置空。自动分片 |
...
- **GET /api/trash**
  ...
  - **响应格式**: 返回纯数组（非对象），trash 行为**纯数据库行**（snake_case），无 `is_series` 等派生字段，`done`/`deleted`/`is_exception` 为整数（0/1），`subtasks`/`search_terms` 为未解析的 JSON 字符串。v3.0 起字段含 `type` / `rrule` / `anchor_date` / `exdates`，无 `repeat_type` 等旧字段。详见 [4.2 节](#42-todo-对象v0-响应格式)。
...
- **POST /api/trash-action**
  ...
  - **Action 详情**:
    | Action | 说明 |
    |--------|------|
    | `RESTORE` | 恢复单条。仅当回收站行仍携带循环属性（`type='recurring'` 且 `parent_id != id`）时处理：同日期已有活跃实例 → 脱钩；模板 `rrule` 非空 → 从 EXDATE 移除并入系列；模板已删除/`rrule` 为空 → 脱钩为单次任务。**不再重建已删除的模板** |
    | `DELETE_PERMANENT` | 永久删除单条。**不可恢复** |
    | `CLEAR_ALL` | 清空回收站。**不可恢复** |
    | `BATCH_RESTORE` | 批量恢复。**自动分片**（每 99 条一组，调用方无需手动拆分）。逻辑同 `RESTORE` |
