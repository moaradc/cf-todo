...
- **PATCH /api/v1/todos/:id/toggle**
  ...
  - **record 写入规则**（与 V0 `TOGGLE_DONE` 一致）：
    ...
    - 切换为未完成（`done: true → false`）：清空实例级 `time_records`。
...
- **POST /api/v1/trash-action**
  ...
  - **Action 详情**:
    ...
    | `BATCH_RESTORE` | 批量恢复。逻辑同 `RESTORE`：同日期已有实例或模板 `repeat_end` 已过期/删除时自动脱钩为单次任务，否则从 EXDATE 移除并入系列 |
...
- **POST /api/todo-action**
  ...
  - **支持的 Action**:
    ...
    | `TOGGLE_DONE` | 切换单个完成状态。`done: false → true` 时可附带 `record`：`s===e` 零耗时仅写实例级，`s<e` 真实耗时实例级 + 模板级双写。`done: true → false` 默认清空实例级 `time_records`；附带 `keepRecords: true`（来自"继续计时"路径）时保留累计记录不清空 |
    | `TIMER_COMPLETE` | 计时"完成"：标记 done=1 + 追加 session 到 `time_records`（实例级 FIFO 30 + 模板级 FIFO 50）。需 `task.id`、`parentId`、`record`，服务端校验时长合理性（1s ~ 7d） |
    | `TIMER_RECORD` | 计时"记录"功能：保存本次 session 到 `time_records`，不标记完成。实例级 FIFO 30 + 模板级 FIFO 50。需 `task.id`、`parentId`、`record`。用于碎片时间多次累计场景 |
