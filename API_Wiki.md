...
- **PUT /api/v1/todos/:id**
  ...
  - **scope 说明**:
    | scope | 行为 |
    |-------|------|
    | `this` | 仅更新此实例（重复任务默认值）。实例脱离系列变为单次任务（对齐 Google Calendar / RFC 5545 "仅此项"语义）：`repeat_type` 强制改为 `none`，`parent_id` 设为自身 `id`，向原系列模板添加 exdate 防止该日期重新生成。**注意：即使用户只改 `text` 等非重复字段，实例也会脱离系列**——这是设计行为，不是 bug。若想原地修改此实例而不脱离系列，调用方需自行重传完整 `repeat_type`/`repeat_interval`/`repeat_custom` 等字段并接受脱离语义；或改用 `scope=all` 改全系列 |
...
- **GET /api/trash**
  ...
  - **响应格式**: 返回纯数组（非对象），trash 行为**纯数据库行**（snake_case），无 `is_series` 等派生字段，`done`/`deleted`/`is_exception` 为整数（0/1），`subtasks`/`search_terms` 为未解析的 JSON 字符串。详见 [4.2 节](#42-todo-对象v0-响应格式)。
...
| scope | 行为 |
|-------|------|
| `this`（默认） | 仅此实例；实例脱离系列变为单次任务（对齐 Google Calendar / RFC 5545 "仅此项"语义）：`repeat_type` 强制改为 `none`，`parent_id` 设为自身 `id`，向原系列模板添加 exdate 防止该日期重新生成。**即使用户只改 `text` 等非重复字段也会脱离系列**（设计行为，非 bug） |
