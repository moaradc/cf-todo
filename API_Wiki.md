...
V0 UPDATE 为 **混合语义**——部分字段 PATCH（未传时从 DB 回退），部分字段全量替换（未传时清空为默认值）。已逐字段端到端验证（基线 todo 带特殊值，V0 UPDATE 只传 `id`+`text`，对比 UPDATE 前后每个字段的 DB 值）：

| 字段 | 未传时行为 | 未传时默认值 |
|------|-----------|-------------|
| `text` | 必填，不可省略 | — |
| `time` / `desc` / `url` / `priority` / `copy_text` | **PATCH**（从 DB 回退） | — |
| `repeat_type` | 全量替换 | `'none'` |
| `repeat_custom` | 全量替换 | `""` |
| `repeat_end` | 全量替换 | `""` |
| `end_time` | 全量替换 | `""` |
| `category_id` | 全量替换 | `""` |
| `repeat_interval` | 全量替换 | `1` |
| `subtasks` | 全量替换 | `[]` |
| `search_terms` | 全量替换 | `[]` |
| `date` | 回退到顶层 `date` 参数 | — |

> **关键差异**：V0 UPDATE 即使只想改 `text`，也必须把 `repeat_type` / `repeat_custom` / `repeat_end` / `end_time` / `category_id` / `repeat_interval` / `subtasks` / `search_terms` 显式传回原值，否则会被清空为默认值。前端 `detail.js` 在每次保存时都会重建 `task` 对象包含所有字段，所以前端用户无感知。**外部脚本调用 V0 时必须传完整 task 对象**，或改用 V1 PUT（PATCH 语义，更友好）。
