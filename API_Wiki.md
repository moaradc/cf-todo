...
- **PATCH /api/v1/todos/:id/toggle**
  ...
  - **record 写入规则**（与 V0 `TOGGLE_DONE` 一致）：
    ...
    - 校验：`s > 0`、`e >= s`、时长 ≤ 7 天、`0 <= p <= (e-s)`。非法 record 不写入，但 `done` 仍切换（保持幂等），响应中 `record_accepted:false` 明确告知调用方 record 被拒绝。
    ...
    - 切换为完成（`done: false → true`）：
      ```json
      {
        ...
        "data": {
          ...
          "done": true,
          "date": "2026-06-30",
          "time_records": [{"s":...,"e":...,"p":0}],
          "record_accepted": true
        }
      }
      ```
      - `date` 字段：碎时记冻结日期或非空时返回
      - `time_records`：**始终返回**（即使为空数组 `[]`），让调用方能对比 record 是否真的写入
      - `record_accepted`：`true` 表示 record 通过校验并已写入；`false` 表示 record 非法（`s>e`、时长>7d、`p>(e-s)`、非对象、缺 `s`/`e`）被拒绝；未传 record 时不返回此字段
    - 切换为未完成（`done: true → false`）：
      ```json
      {"success":true,"data":{"id":"uuid","done":false,"time_records":[]}}
      ```
      - `time_records` 始终为 `[]`（已清空），不返回 `date`/`record_accepted` 字段
...
| `POST /api/category-action` (UPDATE) | 返回 DB 完整对象 + id（无 data 包装） | `{"success":true,"id":"...","name":"...","color":"..."}`（与 V1 PUT 一致，从 DB SELECT 完整 Category 对象） |
...
- **GET /api/todos?date=YYYY-MM-DD**
  - **描述**: 获取指定日期的 Todo 列表（包含自动生成的重复任务实例）。**`date` 参数必填**，缺失返回 400 `{"error":"Date required"}`；非 `YYYY-MM-DD` 格式（如 `invalid`、`20260630`）返回 400 `{"error":"date 格式应为 YYYY-MM-DD，当前值: ..."}`；格式正确但日期不存在（如 `2026-13-45`、`2025-02-29`）返回 400 `{"error":"日期无效: ..."}`。
...
- **POST /api/category-action**
  ...
  - **Action 详情与响应格式**（与 V1 不同，V0 响应不包装在 `data` 字段内）:
    ...
    | `UPDATE` | 更新分类名称或颜色（PATCH 语义，仅传需修改字段）。名称唯一性检查排除自身 | `{"success":true,"id":"...","name":"...","color":"..."}`（返回 DB 中完整的 Category 对象，与 V1 PUT 一致；分类不存在时返回 404 `{"error":"分类不存在"}`） |
...
> **V0 vs V1 差异**：V1 `POST /api/v1/categories/batch` 返回 `{"success":true,"data":{"deleted":N,"chunked":false,"chunkCount":1}}`，V0 仅返回 `{"success":true,"ids":[...]}`。V1 `POST /api/v1/categories/:id` (PUT) 返回完整 Category 对象（包在 `data` 字段内），V0 UPDATE 也返回完整对象但**不包在 `data` 字段内**（直接平铺 `id`/`name`/`color`）。
...
19. **V0 vs V1 响应格式差异**（实测对照）：
    ...
    - V0 `/api/category-action` CREATE 返回 `{"success":true,"id":"...","name":"...","color":"..."}`（无 `data` 包装），UPDATE 返回完整 `{"success":true,"id":"...","name":"...","color":"..."}`（从 DB 读取，与 V1 一致），BATCH_DELETE 返回 `{"success":true,"ids":[...]}`。
