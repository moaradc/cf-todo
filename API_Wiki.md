...
> **v3.0 破坏性变更（重要）**
>
> v3.0 起，所有 API 字段命名**统一为 snake_case**，不再保留 camelCase 兼容层。具体变更：
> - **响应字段**：`is_series`（原 `isSeries`）—— 后端派生字段，由 `repeat_type` 推导，前端不可篡改
> - **task 对象属性**：`copy_text` / `parent_id` / `is_series`（原 `copyText` / `parentId` / `isSeries`）
> - **顶层请求字段**：`done_status` / `keep_records` / `timer_records` / `parent_id`（原 `doneStatus` / `keepRecords` / `timerRecords` / `parentId`）
> - **timer_records 内部字段**：`parent_id`（原 `parentId`）
>
> 旧 camelCase 字段**不再被后端读取**（请求仍会成功，但 camelCase 字段值会被静默丢弃）。请所有调用方尽快迁移到 snake_case。
...
- **GET /api/v1/todos**
  ...
  - **查询参数**:
    ...
    | `offset` | int | 0 | 分页偏移 |
    | `expand` | string | `true` | **v2.7.8.2 新增**。仅 `date` 查询时有效。`true`（默认）：服务端自动展开重复任务模板并持久化实例；`false`：跳过展开，响应附带 `templates` 数组，调用方自行用 ical.js/rrule.js 计算 |
  ...
  - **响应（expand=false，仅 date 查询）**:
    ...
    **注意**：`expand=false` 模式不会自动创建重复实例（不写 D1），调用方拿 `templates` 后需自行判断当天是否应有实例。如需持久化实例，请用默认 `expand=true`。
...
- **POST /api/v1/todos**
  ...
  - **Body**:
    ...
    ```
  - **说明**: 字段名与 V0 Web API 保持一致（snake_case）。`subtasks` 和 `search_terms` 支持纯字符串或 `{text, done}` 对象，纯字符串会自动转为 `{text: "xxx", done: false}`。当 `repeat_type` 不为 `none` 时，会同时创建模板记录。
  - **响应 (201)**:
    ```json
    {
      "success": true,
      "data": { "id": "uuid", "date": "2023-10-01", "text": "Buy milk", "repeat_type": "none", "category_id": "" }
    }
    ```
...
- **PUT /api/v1/todos/:id**
  - **描述**: 更新 Todo。仅传需修改的字段（V1 PATCH 风格）。
  - **Body**: 同创建，所有字段可选。额外支持：
    - `date` (string): 修改日期。对于重复任务配合 `scope=all` 或 `thisAndFuture` 时，会删除并重新生成未来实例。
    - `scope` (string): 用于重复任务，见下方说明。
  - **字段回退规则（v3.0 修复）**：未传的字段（`text` / `time` / `priority` / `desc` / `url` / `copy_text` 等）会从数据库当前值回退，不会被静默清空。例如仅传 `{"time":"14:00"}` 时，其他字段保留原值。
  - **scope 说明**:
    | scope | 行为 |
    |-------|------|
  ...
  - **特殊行为**:
    ...
    - `scope=all` 缺 `task.parent_id` 时，后端自动从 DB 派生（不返回 500）。
  - **响应**:
    ```json
    {
      "success": true,
...
- **POST /api/v1/todos/batch**
  - **描述**: 批量操作 Todo。**v2.7.8.2 起解除 100 条限制，后端自动分片处理，可传任意数量。**
  - **Body**:
    ```json
    {
  ...
  - **自动分片（v2.7.8.2）**: 当 `ids.length > 99` 时，后端按 99 一组自动分片处理，调用方无需手动拆分。分片内单语句失败不阻断整体流程。
...
- **POST /api/v1/categories/batch**
  - **描述**: 批量删除分类。**硬删除**，关联的 Todo 和模板的 `category_id` 自动置空。**v2.7.8.2 起解除 100 条限制，自动分片**。
  - **Body**:
    ```json
    {
      "action": "BATCH_DELETE",
      "ids": ["cat_id1", "cat_id2", /* ... 任意数量 */]
    }
    ```
  - **响应**:
    ```json
...
- **POST /api/v1/trash-action**
  ...
  - **Action 详情**:
    ...
    | `BATCH_RESTORE` | 批量恢复。**v2.7.8.2 起解除 100 条限制，自动分片**。逻辑同 `RESTORE`：同日期已有实例或模板 `repeat_end` 已过期/删除时自动脱钩为单次任务，否则从 EXDATE 移除并入系列 |
    | `BATCH_DELETE_PERMANENT` | 批量永久删除。**v2.7.8.2 起解除 100 条限制，自动分片**。**不可恢复** |
    | `CLEAR_ALL_DATA` | **危险** 清空所有数据（todos、templates、settings、categories），**不可恢复** |
  - **响应**:
    - `RESTORE` / `DELETE_PERMANENT` / `CLEAR_ALL`: `{"success": true}`
    - `BATCH_RESTORE`: `{"success": true, "data": {"restored": 2, "chunked": false, "chunkCount": 1}}`（v2.7.8.2 起 `restored` 为实际恢复行数，附 `chunked`/`chunkCount`）
    - `BATCH_DELETE_PERMANENT`: `{"success": true, "data": {"deleted": 2, "chunked": false, "chunkCount": 1}}`（v2.7.8.2 起 `deleted` 为实际删除行数，附 `chunked`/`chunkCount`）
...
- **POST /api/todo-action**
  ...
  - **Body**:
    ...
    ```
  - **命名规则（v3.0）**：所有字段统一 snake_case，不兼容 camelCase。`task` 对象字段见 [4.2 节](#42-todo-对象-v0-响应格式)。
  - **支持的 Action**:
    | Action | 说明 |
    |--------|------|
    | `CREATE` | 创建任务。若 `repeat_type` 不为 `none`，同时创建模板。`task.parent_id` 可省略（CREATE 时后端始终用 `task.id` 作为 `parent_id`） |
    | `UPDATE` | 更新任务。支持 `scope` 参数控制重复任务更新范围。**字段回退**：未传的字段（`text`/`time`/`priority`/`desc`/`url`/`copy_text` 等）从 DB 当前值回退，不会被清空。`scope=all` 缺 `task.parent_id` 时后端从 DB 派生（不 500） |
...
```json
{
  ...
  "is_series": true,             // 计算字段：repeat_type !== 'none'
...
- **统一 snake_case**（v3.0）：`copy_text`、`repeat_type`、`repeat_end`、`end_time`、`category_id`、`search_terms`、`parent_id`、`recurrence_id`、`is_exception`、`repeat_interval`、`repeat_custom`
...
- **字段回退（v3.0 修复）**：UPDATE 时未传的字段（`text`/`time`/`priority`/`desc`/`url`/`copy_text` 等）从 DB 当前值回退，不会被清空
...
11. **v3.0 破坏性变更**：所有 API 字段统一 snake_case，**移除 camelCase 兼容层**。具体变更：
    - 响应字段 `isSeries` → `is_series`
    - task 对象属性 `copyText` / `parentId` / `isSeries` → `copy_text` / `parent_id` / `is_series`
    - 顶层请求字段 `doneStatus` / `keepRecords` / `timerRecords` / `parentId` → `done_status` / `keep_records` / `timer_records` / `parent_id`
    - `timer_records` 内部字段 `parentId` → `parent_id`
    - 旧 camelCase 字段不再被后端读取（请求仍成功，但 camelCase 字段值被静默丢弃）
12. **v3.0 修复**：V0 `UPDATE` 路径字段回退 bug —— 此前 `readCopyText(task)` 误用调用方原始对象（缺 `copy_text` 时为空），导致按 V1 PATCH 风格仅传部分字段时 `copy_text` 被静默清空。已改为 `readCopyText(original_task)`（含 DB 回退值），未传字段正确保留。同样修复的还包括 11 处 SQL bind 调用。
13. **v3.0 健壮性**：V0 `UPDATE` / `DELETE` 缺 `task.parent_id` 时后端自动从 DB 派生（不返回 500）；`scope=all` 同样适用。
