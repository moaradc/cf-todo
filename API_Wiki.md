...
| 体系 | 文件 | 基础路径 | 鉴权方式 | 用途 |
|------|------|----------|----------|------|
| **V0 (Internal/Web)** | `api.js` | `/api/` | Cookie (`auth_token` + `auth_sig`) | Web 前端专用 |
| **V1 (RESTful)** | `api-v1.js` | `/api/v1/` | API Key 或 Cookie | 外部程序 / OpenClaw 调用 |

### 公开端点（无需鉴权）

以下 V0 端点无需 Cookie 鉴权即可访问：
- `POST /api/login`
- `POST /api/logout`
- `GET /api/hot-search`

V1 端点中，仅 `/api/v1/keys` 使用 Cookie 鉴权，其余均支持 API Key 或 Cookie 双重鉴权。
...
- **POST /api/v1/todos/batch**
  - **描述**: 批量操作 Todo。
  - **已知问题**: 由于路由匹配顺序，`/api/v1/todos/batch` 会被 `/api/v1/todos/:id` 的正则先匹配到（`batch` 被当作 `:id`），导致此端点实际返回 `405 Method Not Allowed`。此为代码 bug，待修复。
  - **Body**:
    ```json
    {
...
基础路径: `/api/`
鉴权: Cookie (`auth_token` + `auth_sig`)，除公开端点外均需鉴权。
...
- **GET /api/stats?start=YYYY-MM-DD&end=YYYY-MM-DD**
  - **描述**: 获取指定时间范围内的原始统计数据（仅未删除的 Todo）。
  - **响应**: 原始行数组，每行包含 `date`、`priority`、`done` 字段。
  - **注意**: V0 返回原始数据，V1 返回聚合统计。如需聚合请使用 V1 Stats API。
