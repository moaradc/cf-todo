...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/import' && request.method === 'POST') {
      ...
      // 导入自定义代码
      if (todo_templates || todos || mode === 'overwrite') {
      }
      if (custom_header !== undefined || custom_content !== undefined) {
        const customStmts = [];
        if (custom_header !== undefined) {
          customStmts.push(env.DB.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('custom_header', ?)").bind(custom_header));
        }
        if (custom_content !== undefined) {
          customStmts.push(env.DB.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('custom_content', ?)").bind(custom_content));
        }
        for (let i = 0; i < customStmts.length; i += 100) {
          await env.DB.batch(customStmts.slice(i, i + 100));
        }
      }
      return new Response(JSON.stringify({ success: true }), { headers: { 'Content-Type': 'application/json' } });
    }
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
    ...
    body:has(.modal-overlay.active, .detail-overlay.active) {
  ...
  <div id="settings-overlay" class="detail-overlay">
    ...
    <div style="flex:1; overflow-y:auto; padding-bottom: 20px;">
      ...
      <div class="settings-card">
          ...
          <p class="settings-text" style="margin-bottom: 12px;">关闭后将不再注入自定义代码，但内容仍会保留在数据库中。</p>
    ...
    function importData(event) {
      ...
      reader.onload = async (e) => {
        try {
          ...
          if (data.custom_header !== undefined || data.custom_content !== undefined) {
            await fetch('/api/custom-code', {
              method: 'POST',
              body: JSON.stringify({
