/*
 * Cloudflare Worker + Supabase Todo App (v0.0.5_supabase_beta: 基于 v2.6.8.2 逻辑适配 Supabase PostgreSQL；支持断点续传导出、分片导入、备份恢复)
...
async function sbFetch(env, method, table, queryParams = {}, body = null, single = false, range = null, upsert = false) {
  ...
  if (range) {
    ...
    if (!headers['Prefer']) headers['Prefer'] = 'count=exact';
  ...
  if (!text) return { data: null, error: null, count: null };
  
  try {
    const json = JSON.parse(text);
    if (res.status >= 400) {
      console.error("Supabase REST Error:", json);
      return { data: null, error: json, count: null };
    }
    let count = null;
...
async function sbCount(env, table, queryParams = {}) {
  const { count } = await sbFetch(env, 'GET', table, { select: 'id', limit: '1', ...queryParams }, null, false, [0, 0]);
  return count || 0;
}

async function clearBackupTablesGlobal(env) {
  try {
    await sbFetch(env, 'DELETE', 'todos_backup', { id: 'neq.impossible_id' });
    await sbFetch(env, 'DELETE', 'todo_templates_backup', { parent_id: 'neq.impossible_id' });
  } catch (e) { console.error("Failed to clear backup tables:", e); }
}
  ...
  async fetch(request, env, ctx) {
    ...
    const isAuthorized = async () => {
      ...
      if (Array.isArray(sessionValue)) {
        ...
        if (!sessionValue.startsWith('[')) {
          if (sessionValue !== cookies.auth_token) return { ok: false };
          sessions = [{ token: sessionValue, ua: '' }];
        } else {
          try {
            sessions = JSON.parse(sessionValue);
    ...
    if (url.pathname === '/api/logout' && request.method === 'POST') {
      if (cookies.auth_token) {
        ...
        if (record && record.value) {
          let sessions = [];
          if (Array.isArray(record.value)) sessions = record.value;
    ...
    if (url.pathname === '/' && request.method === 'GET') {
      ...
      if (authorized && matchedSession) {
        ...
        if (currentUA && matchedSession.ua !== currentUA) {
          ...
          await sbFetch(env, 'POST', 'settings', {}, { key: 'active_session_token', value: updatedSessions }, false, null, true);
    ...
    if (url.pathname === '/api/export' && request.method === 'GET') {
      ...
      if (mode === 'meta') {
        ...
        let todoQuery = { select: 'id', limit: '1' };
        if (incTodos && incTrash) { /* no filter */ }
        else if (incTodos) todoQuery.deleted = 'eq.0';
        else if (incTrash) todoQuery.deleted = 'eq.1';
        else todoQuery.id = 'eq.impossible_id';

        const [todoCount, tplCount] = await Promise.all([
          sbCount(env, 'todos', todoQuery),
          incTodos ? sbCount(env, 'todo_templates', { select: 'parent_id', limit: '1' }) : Promise.resolve(0)
        ]);
        ...
        if (incSettings) {
          const { data: settingsRecord } = await sbFetch(env, 'GET', 'settings', { key: 'eq.app_settings', select: 'value' }, null, true);
          result.settings = settingsRecord?.value || {};
          const { data: headerRecord } = await sbFetch(env, 'GET', 'settings', { key: 'eq.custom_header', select: 'value' }, null, true);
      ...
      if (mode === 'page') {
        ...
        countQuery.select = type === 'templates' ? 'parent_id' : 'id';

        const [total, { data: rows }] = await Promise.all([
          sbCount(env, tableName, countQuery),
          sbFetch(env, 'GET', tableName, queryParams, null, false, [offset, offset + limit - 1])
        ]);
      ...
      if (mode === 'session') {
        ...
        if (action === 'create') {
          ...
          const { data: oldSession } = await sbFetch(env, 'GET', 'export_sessions', { status: 'eq.active' }, null, true);
          if (oldSession) {
            ...
            await sbFetch(env, 'DELETE', 'export_sessions', { id: `eq.${oldSession.id}` });
          }

          await sbFetch(env, 'DELETE', 'export_sessions', { updated_at: `lt.${now - EXPORT_TIMEOUT}` });

          await sbFetch(env, 'POST', 'export_sessions', {}, {
            id, status: 'active', inc_todos: incTodos, inc_trash: incTrash, inc_settings: incSettings,
            total_todos: todoCount, total_templates: tplCount,
            todos_offset: 0, templates_offset: 0, created_at: now, updated_at: now
          });

          return new Response(JSON.stringify({
            sessionId: id,
            totalTodos: todoCount,
            totalTemplates: tplCount
          }), { headers: { 'Content-Type': 'application/json' } });
        }

        if (action === 'status' && sessionId) {
          const { data: session } = await sbFetch(env, 'GET', 'export_sessions', { id: `eq.${sessionId}` }, null, true);
          if (!session) {
        ...
        if (action === 'done' && sessionId) {
          await sbFetch(env, 'DELETE', 'export_sessions', { id: `eq.${sessionId}` });
          return new Response(JSON.stringify({ success: true }), { headers: { 'Content-Type': 'application/json' } });
        }
      ...
      if (mode === 'stream') {
        ...
        const [totalTodos, totalTemplates] = await Promise.all([
          sbCount(env, 'todos', todoQuery),
          incTodos ? sbCount(env, 'todo_templates', { select: 'parent_id', limit: '1' }) : Promise.resolve(0)
        ]);
          ...
          async pull(controller) {
            try {
              if (resumeTodosOffset < totalTodos) {
                ...
                const { data: rows } = await sbFetch(env, 'GET', 'todos', queryParams, null, false, [offset, offset + STREAM_PAGE_SIZE - 1]);

                if (offset === 0 && resumeTemplatesOffset === 0) {
                  let header = '{';
                  if (incSettings) {
                    const { data: settingsRecord } = await sbFetch(env, 'GET', 'settings', { key: 'eq.app_settings', select: 'value' }, null, true);
                    header += '"settings":' + JSON.stringify(settingsRecord?.value || {}) + ',';
                    const { data: headerRecord } = await sbFetch(env, 'GET', 'settings', { key: 'eq.custom_header', select: 'value' }, null, true);
                ...
                for (let i = 0; i < (rows || []).length; i++) {
                  ...
                  const chunk = encoder.encode(prefix + JSON.stringify(exportRow));
                  chunks.push(chunk);
                ...
                if (sessionId && (pageCount % SESSION_UPDATE_INTERVAL === 0 || (rows || []).length < STREAM_PAGE_SIZE)) {
                  await sbFetch(env, 'PATCH', 'export_sessions', { id: `eq.${sessionId}` }, { todos_offset: resumeTodosOffset, updated_at: Date.now() });
                }
              ...
              if (resumeTodosOffset >= totalTodos && resumeTemplatesOffset < totalTemplates) {
                ...
                const { data: rows } = await sbFetch(env, 'GET', 'todo_templates', { select: '*' }, null, false, [offset, offset + STREAM_PAGE_SIZE - 1]);
                ...
                pageCount++;
                if (sessionId && (pageCount % SESSION_UPDATE_INTERVAL === 0 || (rows || []).length < STREAM_PAGE_SIZE)) {
                  await sbFetch(env, 'PATCH', 'export_sessions', { id: `eq.${sessionId}` }, { templates_offset: resumeTemplatesOffset, updated_at: Date.now() });
                }
    ...
    if (url.pathname === '/api/import' && request.method === 'POST') {
      ...
      const buildTodoItems = (items) => (items || []).map((t, idx) => {
        ...
        return {
          id: t.id, parent_id: t.parent_id, date: t.date, text: t.text, time: t.time || '',
          priority: t.priority || 'low',
          repeat: t.repeat !== undefined ? t.repeat : ((t.repeat_type && t.repeat_type !== 'none') ? 1 : 0),
      ...
      const buildTemplateItems = (items) => (items || []).map((t, idx) => {
      ...
      const execBatch = async (items, table) => {
        for (let i = 0; i < items.length; i += BATCH_SIZE) {
          await sbFetch(env, 'POST', table, {}, items.slice(i, i + BATCH_SIZE), false, null, true);
        }
      };
      ...
      try {
        if (phase === 'status') {
          const { data: session } = await sbFetch(env, 'GET', 'import_sessions', { status: 'eq.active' }, null, true);
          if (!session) {
          ...
          const todoBakCount = await sbCount(env, 'todos_backup', { select: 'id', limit: '1' });
          const tplBakCount = await sbCount(env, 'todo_templates_backup', { select: 'parent_id', limit: '1' });
          const hasBackup = todoBakCount > 0 || tplBakCount > 0;
        ...
        if (phase === 'abort') {
          ...
          const { data: session } = await sbFetch(env, 'GET', 'import_sessions', { id: `eq.${importId}` }, null, true);
          ...
          if (session.mode === 'overwrite') {
              ...
              const { data: backupTodos } = await sbFetch(env, 'GET', 'todos_backup', { select: '*' });
              if (backupTodos && backupTodos.length > 0) {
                for (let i = 0; i < backupTodos.length; i += BATCH_SIZE) {
                  await sbFetch(env, 'POST', 'todos', {}, backupTodos.slice(i, i + BATCH_SIZE), false, null, true);
                }
              }

              const { data: backupTpls } = await sbFetch(env, 'GET', 'todo_templates_backup', { select: '*' });
              if (backupTpls && backupTpls.length > 0) {
                for (let i = 0; i < backupTpls.length; i += BATCH_SIZE) {
                  await sbFetch(env, 'POST', 'todo_templates', {}, backupTpls.slice(i, i + BATCH_SIZE), false, null, true);
                }
              }

              await clearBackupTablesGlobal(env);
              } catch (e) {
                return errResp('恢复备份失败: ' + e.message, 500);
              }
          }

          await sbFetch(env, 'DELETE', 'import_sessions', { id: `eq.${importId}` });
        ...
        if (phase === 'init') {
          ...
          const { data: oldSession } = await sbFetch(env, 'GET', 'import_sessions', { status: 'eq.active' }, null, true);
          if (oldSession) {
            const now = Date.now();
            const TIMEOUT = 10 * 60 * 1000;
            ...
            await sbFetch(env, 'DELETE', 'import_sessions', { id: `eq.${oldSession.id}` });
          }
          ...
          if (mode === 'overwrite') {
            try {
              ...
              const { data: allTodos } = await sbFetch(env, 'GET', 'todos', { select: '*' });
              if (allTodos && allTodos.length > 0) {
                for (let i = 0; i < allTodos.length; i += BATCH_SIZE) {
                  await sbFetch(env, 'POST', 'todos_backup', {}, allTodos.slice(i, i + BATCH_SIZE));
                }
              }

              const { data: allTpls } = await sbFetch(env, 'GET', 'todo_templates', { select: '*' });
              if (allTpls && allTpls.length > 0) {
                for (let i = 0; i < allTpls.length; i += BATCH_SIZE) {
                  await sbFetch(env, 'POST', 'todo_templates_backup', {}, allTpls.slice(i, i + BATCH_SIZE));
                }
              }
              ...
              await sbFetch(env, 'POST', 'import_sessions', {}, {
                id: importId, mode: 'overwrite', status: 'active', started_at: now, updated_at: now
              });
            } catch (backupErr) {
                try {
                  const { data: backupTodos } = await sbFetch(env, 'GET', 'todos_backup', { select: '*' });
            ...
            await sbFetch(env, 'POST', 'import_sessions', {}, {
              id: importId, mode: 'merge', status: 'active', started_at: now, updated_at: now
            });
          }
          ...
          const { data: session } = await sbFetch(env, 'GET', 'import_sessions', { id: `eq.${importId}`, status: 'eq.active' }, null, true);
          if (!session) return errResp('无效或已过期的导入会话');

          const chunk = body.data || [];
          const type = body.type;

          await sbFetch(env, 'PATCH', 'import_sessions', { id: `eq.${importId}` }, { updated_at: Date.now() });
          ...
          let items;
          if (type === 'templates') {
            items = buildTemplateItems(chunk);
          } else {
            items = buildTodoItems(chunk);
          }

          const tableName = type === 'templates' ? 'todo_templates' : 'todos';
          await execBatch(items, tableName);
          ...
          if (body.custom_header !== undefined || body.custom_content !== undefined) {
            if (body.custom_header !== undefined) {
              await sbFetch(env, 'POST', 'settings', {}, { key: 'custom_header', value: body.custom_header }, false, null, true);
            }
            if (body.custom_content !== undefined) {
              await sbFetch(env, 'POST', 'settings', {}, { key: 'custom_content', value: body.custom_content }, false, null, true);
            }
          }
    ...
    if (url.pathname === '/api/import-backup') {
      ...
      try {
        if (action === 'query') {
          const todoBakCount = await sbCount(env, 'todos_backup', { select: 'id', limit: '1' });
          const tplBakCount = await sbCount(env, 'todo_templates_backup', { select: 'parent_id', limit: '1' });
          const hasTodoBak = todoBakCount > 0;
          const hasTplBak = tplBakCount > 0;
          return new Response(JSON.stringify({
        ...
        if (action === 'restore') {
          ...
          try {
            ...
            const { data: backupTodos } = await sbFetch(env, 'GET', 'todos_backup', { select: '*' });
            if (backupTodos && backupTodos.length > 0) {
              for (let i = 0; i < backupTodos.length; i += 100) {
                await sbFetch(env, 'POST', 'todos', {}, backupTodos.slice(i, i + 100), false, null, true);
              }
            }

            const { data: backupTpls } = await sbFetch(env, 'GET', 'todo_templates_backup', { select: '*' });
            if (backupTpls && backupTpls.length > 0) {
              for (let i = 0; i < backupTpls.length; i += 100) {
                await sbFetch(env, 'POST', 'todo_templates', {}, backupTpls.slice(i, i + 100), false, null, true);
              }
            }
    ...
    if (url.pathname === '/api/settings' && request.method === 'POST') {
      ...
      await sbFetch(env, 'POST', 'settings', {}, { key: 'app_settings', value: data }, false, null, true);
      return new Response(JSON.stringify({ success: true }), { headers: { 'Content-Type': 'application/json' } });
    }
...
<body>
  ...
  <div id="settings-overlay" class="detail-overlay">
    ...
    <div style="flex:1; overflow-y:auto; padding-bottom: 20px;">
      ...
      <div class="settings-card">
          <p class="settings-text" style="margin-bottom:5px;"><strong>当前版本:</strong> v0.0.5_supabase_beta</p>
          <p class="settings-text" style="margin-bottom: 5px;"><strong>底层架构:</strong> Cloudflare Worker + Supabase PostgreSQL</p>
